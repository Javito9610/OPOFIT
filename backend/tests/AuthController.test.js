...
const { registrar, login, loginConGoogle } = require('../src/controllers/AuthController');
...
describe('AuthController', () => {
    ...
    describe('registrar', () => {
        ...
        test('debería devolver 201 si el registro es exitoso', async () => {
            ...
            AuthService.registrar.mockResolvedValue({ userId: 1 });
    ...
    describe('login', () => {
        ...
        test('debería devolver 200 con token si las credenciales son correctas', async () => {
            req.body = { email: 'test@test.com', password: '123456' };
            const usuario = { id_usuario: 1, email: 'test@test.com', nombre: 'Test' };
        ...
        test('debería devolver 500 si JWT_SECRET no está definido', async () => {
            ...
            req.body = { email: 'test@test.com', password: '123456' };
            AuthService.login.mockResolvedValue({ id_usuario: 1, email: 'test@test.com' });
    ...
    describe('loginConGoogle', () => {
        ...
        test('debería devolver 400 si falta email', async () => {
            req.body = { googleToken: 'token-google' };
        ...
        test('debería devolver 200 con token si Google login es exitoso', async () => {
            req.body = { googleToken: 'token-google', email: 'test@gmail.com', nombre: 'Test' };
            const usuario = { id_usuario: 1, email: 'test@gmail.com', nombre: 'Test' };
        ...
        test('debería devolver 500 sin detalles del error interno', async () => {
            req.body = { googleToken: 'token-invalido', email: 'test@gmail.com' };
        ...
        test('debería usar nombre por defecto si no se proporciona', async () => {
            req.body = { googleToken: 'token-google', email: 'test@gmail.com' };
            const usuario = { id_usuario: 1, email: 'test@gmail.com' };
            AuthService.loginConGoogle.mockResolvedValue(usuario);
            jwt.sign.mockReturnValue('jwt-token');

            await loginConGoogle(req, res);

            expect(AuthService.loginConGoogle).toHaveBeenCalledWith(
                'token-google',
                'test@gmail.com',
                'Usuario Google'
            );
