...
describe('GruposService', () => {
  ...
  test('crearGrupo crea grupo y asigna ADMIN al creador', async () => {
    const { id_grupo: idGrupo } = await GruposService.crearGrupo(1, {
      nombre: 'Grupo PN',
      descripcion: 'Entreno',
      idOposicion: 1
    });
  ...
  test('crearGrupo fitness sin oposicion', async () => {
    ...
    expect(grupos[0].id_grupo).toBe(idGrupo);
    expect(grupos[0].id_oposicion).toBeNull();
  });
  ...
  test('unirse agrega miembro', async () => {
    const { id_grupo: idGrupo } = await GruposService.crearGrupo(1, { nombre: 'G', idOposicion: 1 });
